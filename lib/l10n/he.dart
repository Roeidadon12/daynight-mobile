const Map<String, String> heTexts = {
  'home': 'בית',
  'search': 'חיפוש',
  'ticket': 'כרטיס',
  'editing': 'עריכה',
  'welcome': 'ברוכים הבאים ל-Day Night',
  'day_night_home': 'דף הבית Day Night',
  'upcoming-events-label': 'אירועים קרובים',
  'view-all': 'הצג הכל',
  'search-events': 'חפש אירועים',
  'criteria-date': 'תאריך',
  'criteria-price': 'מחיר',
  'criteria-event-type': 'סוג אירוע',
  'filter-by-date': 'סינון תאריך',
  'no-results': 'אין תוצאות',
  'clear': 'נקה',
  'search-button': 'חיפוש',
  'add-to-calendar': 'הוסף ליומן',
  'navigate-to': 'נווט',
  'currency': '₪',
  'to-buy-ticket': 'לרכישת כרטיס',
  'filter-by-price': 'סינון מחיר',
  'currency-option': 'בחר מטבע',
  'from-price': 'ממחיר',
  'to-price': 'למחיר',
  'apply': 'החל',
  'reset': 'איפוס',
  'ILS': 'שקל חדש (₪)',
  'USD': 'דולר אמריקאי (\$)',
  'EUR': 'יורו (€)',
  'GBP': 'לירה שטרלינג (£)',
  'buy-tickets': 'רכישת כרטיסים',
  'no-events-available': 'אין אירועים זמינים כרגע',
  'no-today-events': 'אין אירועים להיום',
  'no-week-events': 'אין אירועים השבוע',
  'no-category-events': 'אין אירועים בקטגוריה זו',
  'no-upcoming-events': 'אין אירועים קרובים',
  'week-events': 'אירועי השבוע',
  'today-events': 'אירועי היום',
  'sold-out': 'אזל מהמלאי',
  'proceed-to-payment': 'המשך לתשלום',
  'processing-fee': 'דמי טיפול',
  'currency-symbol': '₪',
  'add-items': 'הוסף פריטים',
  'gender-male': 'זכר',
  'gender-female': 'נקבה',
  'gender-other': 'אחר',
  'first-name': 'שם פרטי',
  'last-name': 'שם משפחה',
  'id-number': 'מספר תעודת זהות',
  'date-of-birth': 'תאריך לידה',
  'gender': 'מגדר',
  'personal-info': 'פרטי קשר',
  'participant-info': 'פרטי משתתף',
  'purchaser-info': 'פרטי רוכש',
  'total-participants': 'סך כל המשתתפים',
  'total-amount': 'סכום כולל',
  'to-payment-of': 'לתשלום של',
  'participant': 'משתתף',
  'phone-number': 'מספר טלפון',
  'id-card-image': 'יש לצרף תעודת זהות',
  'please-fill-in-all-required-fields': 'אנא מלא את כל השדות הנדרשים',
  'event-details': 'פרטי האירוע',
  'organizer': 'מארגן',
  'contact-organizer': 'צור קשר עם המארגן',
  'select-image-source': 'בחר מקור תמונה',
  'camera': 'מצלמה',
  'gallery': 'גלריה',
  'promo-code': 'קוד הנחה',
  'ok': 'אישור',
  'coupon-applied-successfully': 'הקופון הוזן בהצלחה',
  'invalid-coupon': 'קופון לא חוקי',
  'order-summary': 'סיכום הזמנה',
  'subtotal': 'סכום ביניים',
  'discount': 'הנחה',
  'total': 'סה"כ',
  'image-selected': 'תמונה נבחרה',
  'current-image': 'תמונה נוכחית',
  'tap-to-view-full-image': 'הקש כדי לצפות בתמונה מלאה',
  'minimum-age-required': 'גיל מינימלי נדרש: ',
  'payment-with-credit': 'תשלום בכרטיס אשראי',
  'subscribe-newsletter-daynight': 'אני מאשר/ת קבלת חומר שיווקי מ-Day Night',
  'subscribe-newsletter-organizer': 'אני מאשר/ת קבלת חומר שיווקי של',
  'agree-to-terms': 'אני מסכים לתנאים ולהגבלות',
  'by-clicking-payment-agree-to': 'על ידי לחיצה על תשלום, אני מסכים ל',
  'terms-and-conditions': 'תנאים והגבלות',
  'and': 'ו',
  'privacy-policy': 'מדיניות פרטיות',
  'by-pressing-continue-approve': 'על ידי לחיצה על כפתור המשך אתה מאשר את ',
  'terms-and-conditions-link': 'התנאים וההגבלות',
  'privacy-policy-link': 'מדיניות הפרטיות',
  'payment-success': 'תשלום בוצע בהצלחה',
  'enjoy': 'תהנו',
  'purchase-completed-successfully': 'הרכישה הושלמה בהצלחה!',
  'tickets-sent-to-email': 'הכרטיסים נשלחו לכתובת האימייל שלך.',
  'to-my-tickets': 'לכרטיסים שלי',
  'my-tickets': 'הכרטיסים שלי',
  'upcoming-events': 'אירועים קרובים',
  'closed-events': 'אירועים סגורים',
  'favorite-events': 'אירועים מועדפים',
  'my-events': 'האירועים שלי',
  'add-event': 'הוספת אירוע',
  'my-editing': 'האירועים שלי',
  'drafts': 'טיוטות',
  'published': 'פורסם',
  'create-first-event': 'צרו את האירוע הראשון שלכם כדי להתחיל',
  'create-new-event': 'יצירת אירוע',
  'create-event': 'צור אירוע',
  'create-event-basic-info': 'פרטים על האירוע',
  'create-event-basic-info-description': 'הצעד הראשון לאירוע מוצלח: הגדר את הבסיס.',
  'create-event-design': 'תיאור האירוע',
  'create-event-design-description': 'בוא נעצב את האווירה שמדברת אל הקהל שלך, הרושם כן קובע!',
  'create-event-details': 'הגדרות מתקדמות',
  'create-event-details-description': 'קח את האירוע שלך קדימה עם הגדרות מתקדמות שיעזרו לך להצליח כמו הגדולים!',
  'create-event-tickets-pricing': 'פרטי חשבון',
  'create-event-tickets-pricing-description': 'לאן תרצה לקבל את הכסף?',
  'event-name': 'שם האירוע',
  'enter-event-name': 'שם הרשמי של האירוע שיופיע לכולם',
  'start-time': 'שעת התחלה',
  'end-time': 'שעת סיום',
  'select-start-time': 'בחר שעת התחלה',
  'select-end-time': 'בחר שעת סיום',
  'end-time-must-be-after-start-time': 'שעת הסיום חייבת להיות אחרי שעת ההתחלה',
  'minimal-age': 'גיל מינימלי',
  'enter-minimal-age': '',
  'invalid-age': 'אנא הזן גיל חוקי (0-120)',
  'category': 'קטגוריה',
  'location': 'מיקום',
  'enter-event-location': 'הזן מיקום האירוע',
  'event-name-required': 'שם האירוע הוא שדה חובה',
  'location-required': 'מיקום הוא שדה חובה',
  'create-event-continue-to-description': 'המשך לתיאור האירוע',
  'event-timezone': 'איזור זמן',
  'event-currency': 'מטבע',
  'event-language': 'שפה',
  'date-format': 'dd/MM/yyyy',
  'time-format': 'HH:mm',
  'datetime-format': 'dd/MM/yyyy HH:mm',
  'create-event-main-image': 'תמונת ראשית של האירוע (ניתן לשנות בכל עת)',
  'create-event-description': 'תיאור האירוע',
  'create-event-description-instructions': 'כתוב תיאור שיגרום להם לרצות להגיע ומה יהיה באירוע',
  'description-minimum-characters': '(נדרש מינימום 30 תווים)',
  'characters': 'תווים',
  'tap-to-edit': 'לחץ לעריכת התיאור',
  'save': 'שמור',
  'discard': 'בטל',
  'unsaved-changes': 'שינויים לא נשמרו',
  'unsaved-changes-message': 'האם ברצונך לשמור את השינויים?',
  'create-event-continue-to-advanced-settings': 'המשך להגדרות מתקדמות',
  'organizer-name': 'שם המארגן',
  'enter-organizer-name': 'הזן שם המארגן',
  'organizer-name-required': 'שם המארגן הוא שדה חובה',
  'email-for-contact': 'אימייל ליצירת קשר',
  'enter-email-for-contact': 'הזן אימייל ליצירת קשר',
  'email-for-contact-required': 'אימייל ליצירת קשר הוא שדה חובה',
  'organizer-identification-number': 'מספר זיהוי מארגן',
  'enter-organizer-identification-number': 'הזן מספר זיהוי מארגן',
  'organizer-identification-number-required': 'מספר זיהוי מארגן הוא שדה חובה',
  'bank-number': 'מספר בנק',
  'enter-bank-number': 'הזן מספר בנק',
  'bank-number-required': 'מספר בנק הוא שדה חובה',
  'branch': 'סניף',
  'enter-branch': 'הזן סניף',
  'branch-required': 'סניף הוא שדה חובה',
  'bank-account-number': 'מספר חשבון בנק',
  'enter-bank-account-number': 'הזן מספר חשבון בנק',
  'bank-account-number-required': 'מספר חשבון בנק הוא שדה חובה',
  'account-holder-name': 'שם בעל החשבון',
  'enter-account-holder-name': 'הזן שם בעל החשבון',
  'account-holder-name-required': 'שם בעל החשבון הוא שדה חובה',
  'meta-pixel': 'Meta Pixel',
  'enter-meta-pixel': 'הזן Meta Pixel',
  'tiktok-pixel': 'TikTok Pixel',
  'enter-tiktok-pixel': 'הזן TikTok Pixel',
  'ga4-analytics': 'GA-4 Analytics',
  'enter-ga4-analytics': 'הזן GA-4 Analytics',
  'url-address': 'כתובת URL',
  'your-event-name': 'שם האירוע שלך',
  'private-event': 'אירוע פרטי',
  'private-event-description': 'רק משתמשים מוזמנים יכולים לראות את האירוע הזה',
  'i-agree-to-the': 'אני מסכים ל',
  'terms-of-service': 'תנאי השירות',
  'users-tracking': 'מעקב משתמשים',
  'tracking-field-1': 'שדה מעקב 1',
  'enter-tracking-field-1': 'הזן שדה מעקב 1',
  'tracking-field-2': 'שדה מעקב 2',
  'enter-tracking-field-2': 'הזן שדה מעקב 2',
  'tracking-field-3': 'שדה מעקב 3',
  'enter-tracking-field-3': 'הזן שדה מעקב 3',
  'tracking-field-4': 'שדה מעקב 4',
  'enter-tracking-field-4': 'הזן שדה מעקב 4',
  'free-event': 'אירוע חינמי',
  'free-event-description': 'האירוע הזה חינמי להשתתפות',
  'ticket-types': 'סוגי כרטיסים',
  'add-ticket-type': 'הוספת סוג כרטיס',
  'ticket-type': 'סוג כרטיס',
  'ticket-name': 'שם הכרטיס',
  'enter-ticket-name': 'הזן שם כרטיס',
  'ticket-name-required': 'שם הכרטיס נדרש',
  'price': 'מחיר',
  'price-required': 'מחיר נדרש',
  'price-must-be-number': 'המחיר חייב להיות מספר',
  'quantity': 'כמות',
  'quantity-required': 'כמות נדרשת',
  'quantity-must-be-positive': 'הכמות חייבת להיות חיובית',
  'description-optional': 'תיאור (אופציונלי)',
  'enter-ticket-description': 'הזן תיאור כרטיס',
  'no-ticket-types': 'לא נוספו סוגי כרטיסים עדיין',
  'add-first-ticket-type': 'הוסף סוג כרטיס ראשון',
  'remove': 'הסר',
  'previous': 'קודם',
  'event-created': 'האירוע נוצר',
  'event-ready-to-share': 'האירוע שלכם מוכן לשיתוף עם העולם!',
  'congratulations': 'ברכותינו',
  'event-created-successfully': 'וואו, סיימתם בהצלחה!',
  'event-is-now-live': 'האירוע שלכם אוטוטו באוויר',
  'event-is-live-our-turn': 'עכשיו תורנו לעבור על הפרטים ותקבלו עדכון מאיתנו ב-SMS ובמייל',
  'share-event-url': 'שתפו את האירוע שלכם באמצעות הקישור למטה:',
  'url-copied': 'הקישור הועתק ללוח',
  'copy-url': 'העתק קישור',
  'manage-my-event': 'נהל את האירוע שלי',
  'back-to-events': 'חזרה לאירועים',
  'to-main-screen': 'למסך הראשי',
  'connect-subscribe': 'התחברות / רשומה',
  'connect-with-google': 'התחבר באמצעות Google',
  'connect-with-apple': 'התחבר באמצעות Apple',
  'phone-required': 'מספר טלפון נדרש',
  'phone-min-length': 'מספר הטלפון קצר מדי',
  'send-sms-code': 'שלח קוד באמצעות SMS',
  'sms-verification': 'אימות SMS',
  'verify-phone-number': 'אימות מספר טלפון',
  'verification-code': 'קוד אימות',
  'sms-sent-to': 'קוד SMS נשלח אל',
  'verify': 'אמת',
  'resend-code': 'שלח קוד שוב',
  'change-phone-number': 'שנה מספר טלפון',
  'sms-code-required': 'קוד אימות נדרש',
  'sms-code-invalid': 'אנא הכנס קוד בן 6 ספרות תקין',
  'sms-send-failed': 'שליחת קוד SMS נכשלה',
  'sms-verification-failed': 'אימות SMS נכשל',
  'back-to-phone': 'חזור למספר טלפון',
  'login': 'התחברות',
  'login-title': 'ברוכים השבים',
  'login-subtitle': 'התחברו לחשבון שלכם כדי להמשיך',
  'login-failed': 'ההתחברות נכשלה. אנא בדקו את הפרטים שלכם.',
  'Login-with-Password': 'השתמש בסיסמה',
  'login-register': 'התחברות/הרשמה',
  'email': 'דואר אלקטרוני',
  'email-optional': 'דואר אלקטרוני (אופציונלי)',
  'email-required': 'דואר אלקטרוני נדרש',
  'email-invalid': 'אנא הכניסו כתובת דואר אלקטרוני תקינה',
  'password': 'סיסמה',
  'password-required': 'סיסמה נדרשת',
  'password-min-length': 'הסיסמה חייבת להכיל לפחות 6 תווים',
  'continue-as-guest': 'המשיכו כאורח',
  'or': 'או',
  'no-account': 'אין לכם חשבון?',
  'create-account': 'יצירת חשבון',
  'registration-title': 'הצטרפו ל-Day Night',
  'registration-subtitle': 'צרו את החשבון שלכם כדי להתחיל',
  'registration-failed': 'רישום נכשל. אנא נסו שוב.',
  'full-name': 'שם מלא',
  'full-name-required': 'שם מלא נדרש',
  'full-name-min-length': 'השם המלא חייב להכיל לפחות 2 תווים',
  'phone-invalid': 'אנא הכניסו מספר טלפון תקין',
  'id-number-required': 'מספר תעודת זהות נדרש',
  'id-number-invalid': 'אנא הכניסו מספר תעודת זהות תקין',
  'date-of-birth-required': 'תאריך לידה נדרש',
  'address': 'כתובת',
  'optional': 'אופציונלי',
  'confirm-password': 'אישור סיסמה',
  'confirm-password-required': 'אנא אשרו את הסיסמה',
  'passwords-do-not-match': 'הסיסמאות אינן תואמות',
  'guest-mode': 'מצב אורח',
  'guest-mode-description': 'התחברו כדי לגשת לכרטיסים שלכם, ליצור אירועים ולקבל המלצות מותאמות אישית.',
  'logout': 'התנתקות',
  'user-status-connected': 'מחובר',
  'user-status-guest': 'אורח',
  'user-status-unknown': 'לא ידוע',
  'phone-number-required-for-identification': 'מספר טלפון נדרש לצורך זיהוי',
  'upload-image': 'העלה תמונה',
  'image-size-recommendation': 'גודל מומלץ: 1080x1080 פיקסלים, עד 5MB',
  'take-photo': 'צלם תמונה',
  'choose-from-gallery': 'בחר מהגלריה',
  'cancel': 'בטל',
  'image-selection-error': 'שגיאה בבחירת תמונה. אנא נסו שוב.',
  'select-image': 'בחר תמונה',
  'organizer-enter-permits': 'אישור כניסה',
  'organizer-scan': 'סריקה',
  'organizer-edit-event': 'עריכה',
  'organizer-filter-active': 'פעילים',
  'organizer-filter-non-active': 'לא פעילים',
  'organizer-empty-active-events': 'אין אירועים פעילים',
  'organizer-empty-non-active-events': 'אין אירועים לא פעילים',
  'edit-event-team': 'צוות',
  'edit-event-links': 'קישורים',
  'edit-event-coupons': 'קופונים',
  'edit-event-cashier': 'קופה',
  'detail-event-team': 'צוות',
  'detail-event-links': 'קישורים',
  'detail-event-coupons': 'קופונים',
  'detail-event-cashier': 'קופה',
  'search-participants': 'חיפוש משתתפים',
  'waiting-participants': 'ממתינים',
  'summary-status-approved': 'אושרו',
  'summary-status-dropouts': 'נוטשים',
  'summary-status-rejected': 'נדחו',
  'summary-status-hidden': 'מוסתרים',
  'total-earnings': 'סך הכנסות',
  'special-graphs-sales-by-gender': 'מכירת לפי מין',
  'special-graphs-women': 'נשים',
  'special-graphs-men': 'גברים',
  'special-graphs-total': 'סה\"כ',
  'special-graphs-participants': 'משתתפים',
  'event-edit-general': 'כללי',
  'event-edit-media': 'מדיה',
  'event-edit-advanced': 'מתקדם',
  'event-edit-bank-details': 'פרטי בנק',
};